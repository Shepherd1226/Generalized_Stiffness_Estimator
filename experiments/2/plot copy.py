...
def evaluate(time_values,F_real_values,F_goal_values,k_real_values,k_est_values,pos_values,k_m,plot=True):
    # 计算指标
    ...
    if plot:
        ...
        plt.subplot(1, 3, 1)
        plt.plot(time_values, F_real_values,color='red')#, label='F_N')
        ...
        plt.subplot(1, 3, 2)
        plt.plot(pos_values, F_real_values,color='orange')#, label='F_real')
        ...
        plt.subplot(1, 3, 3)
        plt.plot(time_values, k_m, label='k_m',color='gray')
        plt.plot(time_values, k_est_values, label='k_l')        
        plt.plot(time_values, k_real_values, label='k')
...
if __name__=='__main__':
    # 读取数据
    data_file = curr_path+'/output_kl.txt'
    ...
    # 读取数据
    data_file = curr_path+'/output_km.txt'
    with open(data_file, "r") as file:
        lines = file.readlines()
    ...
    for line in lines:
        # 假设数据之间使用空格分隔
        values = line.strip().split()
        k_m_values.append(float(values[3]))

    
    evaluate(time_values,F_real_values,F_goal_values,k_real_values,k_est_values,pos_values,k_m_values,plot=True)
