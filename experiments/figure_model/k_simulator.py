...
class F_Pos_simulator():
    ...
    def generate(self,plot=False):
        ...
        # 生成对应的导数值
        self.log_der_values = (np.random.uniform(np.log(3), np.log(1000), size=self.num_points))

        self.F_drifter=np.empty(self.num_points,dtype=object)
        self.log_der_drifter=np.empty(self.num_points,dtype=object)
        for i in range(self.num_points):
            self.F_drifter[i]=Drifter(self.F_values[i],min(self.F_values[i]*1.1,10.0,self.F_values[i]+0.5),max(self.F_values[i]*0.9,0.0,self.F_values[i]-0.5),0.1,-0.1)
